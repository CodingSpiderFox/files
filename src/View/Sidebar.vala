...
public class Marlin.Sidebar : Marlin.AbstractSidebar {
    ...
    private void construct_tree_view () {
        ...
        tree_view.set_size_request (app_settings.get_int ("minimum-sidebar-width"), -1);
...
/* BOOKMARK/SHORTCUT FUNCTIONS */
    ...
    private void expander_update_pref_state (PlaceType type, bool flag) {
        ...
        switch (type) {
            case PlaceType.NETWORK_CATEGORY:
                if (flag != app_settings.get_boolean ("sidebar-cat-network-expander")) {
                    app_settings.set_boolean ("sidebar-cat-network-expander", flag);
                }
                break;
            case PlaceType.STORAGE_CATEGORY:
                if (flag != app_settings.get_boolean ("sidebar-cat-devices-expander")) {
                    app_settings.set_boolean ("sidebar-cat-devices-expander", flag);
                }
                break;
            case PlaceType.BOOKMARKS_CATEGORY:
                if (flag != app_settings.get_boolean ("sidebar-cat-personal-expander")) {
                    app_settings.set_boolean ("sidebar-cat-personal-expander", flag);
                }
                break;
        }
    }
    ...
    private void expander_init_pref_state (Gtk.TreeView tree_view) {
        ...
        if (app_settings.get_boolean ("sidebar-cat-personal-expander")) {
        ...
        if (app_settings.get_boolean ("sidebar-cat-devices-expander")) {
            tree_view.expand_row (path, false);
        } else {
            tree_view.collapse_row (path);
        }
        ...
        if (app_settings.get_boolean ("sidebar-cat-network-expander")) {
            tree_view.expand_row (path, false);
        } else {
            tree_view.collapse_row (path);
        }
